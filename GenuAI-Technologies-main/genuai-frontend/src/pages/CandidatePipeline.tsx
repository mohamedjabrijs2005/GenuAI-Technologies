...
export default function CandidatePipeline({ user, onLogout, onInterview }: Props) {
  const [stage, setStage] = useState<Stage>('module7'); // Temp preview
  const [completedModules, setCompletedModules] = useState<number[]>([1,2,3,4,5,6]); // Temp preview
  const [moduleData, setModuleData] = useState<Record<number,any>>({
    1: { overall: 85 }, 2: { overall: 92 }, 3: { overall: 78 }, 
    4: { overall: 95 }, 5: { overall: 88 }, 6: { overall: 90 }
  }); // Temp preview
