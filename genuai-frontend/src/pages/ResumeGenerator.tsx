...
- Links & Profiles:
  ...
  - Portfolio/Other: ${portfolio || "Not provided"}
  ...
  return (
    <div className="resume-gen-page h-screen bg-background quantum-gradient flex flex-col overflow-hidden">
      <style>{`
        ...
        .markdown-body { font-family: 'Inter', sans-serif; color: #111827; text-align: left; }
        .markdown-body h1 { font-size: 28px; margin-bottom: 12px; color: #111827; text-align: center; border-bottom: none; font-weight: 900; letter-spacing: -0.5px; line-height: 1.2; }
        .markdown-body h2 { font-size: 15px; margin-top: 28px; margin-bottom: 12px; color: #1f2937; border-bottom: 2px solid #e5e7eb; padding-bottom: 6px; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 800; }
        .markdown-body h3 { font-size: 14px; margin-top: 16px; margin-bottom: 6px; color: #111827; font-weight: 700; }
        .markdown-body p { margin-bottom: 12px; line-height: 1.6; color: #374151; font-size: 13px; font-weight: 500; }
        .markdown-body ul { margin-bottom: 16px; padding-left: 24px; color: #374151; font-size: 13px; }
        .markdown-body li { margin-bottom: 6px; line-height: 1.6; }
        .markdown-body strong { color: #111827; font-weight: 700; }
        .markdown-body em { font-style: italic; color: #4b5563; font-size: 12px; }
        .markdown-body a { color: #2563eb; text-decoration: none; font-weight: 600; }
      `}</style>
      ...
      <div className="glass border-b border-surface-container px-3 sm:px-6 h-14 sm:h-16 flex items-center justify-between shrink-0 sticky top-0 z-40 gap-2">
        <div className="flex items-center gap-2 min-w-0">
          <div className="font-black text-xs sm:text-sm text-on-surface flex items-center gap-1.5 truncate">
            <span>📄</span> <span className="truncate">ATS Resume Builder</span>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-lg md:p-xl space-y-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-sm">
               <div>
                 ...
                 <input value={candidateName} onChange={e=>setCandidateName(e.target.value)} placeholder="Your Full Name" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
               </div>
               <div>
                 <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-xs block">Email Address *</label>
                 <input value={candidateEmail} onChange={e=>setCandidateEmail(e.target.value)} placeholder="Your Email Address" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
               </div>
               <div>
                 <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-xs block">Phone Number</label>
                 <input value={phone} onChange={e=>setPhone(e.target.value)} placeholder="Your Phone Number" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
               </div>
               <div>
                 <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-xs block">Location</label>
                 <input value={location} onChange={e=>setLocation(e.target.value)} placeholder="City, Country" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
               </div>
            </div>
            
            <h2 className="text-title-sm font-black text-on-surface border-b border-surface-container pb-xs mt-xl mb-sm">Links & Profiles</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-sm">
              <div>
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-xs block">LinkedIn</label>
                <input value={linkedin} onChange={e=>setLinkedin(e.target.value)} placeholder="LinkedIn Profile URL" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-xs block">GitHub</label>
                <input value={github} onChange={e=>setGithub(e.target.value)} placeholder="GitHub Profile URL" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
              </div>
              <div>
                <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-xs block">Portfolio / Other</label>
                <input value={portfolio} onChange={e=>setPortfolio(e.target.value)} placeholder="Portfolio or Website URL" className="w-full p-sm bg-background border border-surface-container rounded-xl text-sm font-medium text-on-surface outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 transition-all" />
              </div>
            </div>
