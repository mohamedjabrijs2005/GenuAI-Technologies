...
interface Props {
  user: any;
  onSelect: (path: 'practice' | 'search' | 'career-profile' | 'companies' | 'my-assessment') => void;
  onLogout: () => void;
}

export default function PathSelection({ user, onSelect, onLogout }: Props) {
  const [hovered, setHovered] = useState<string | null>(null);
  const [savedSelections, setSavedSelections] = useState<CompanyRoleSelectionItem[]>([]);
  ...
  return (
    <div className="min-h-screen bg-background quantum-gradient font-body-base text-on-background relative overflow-hidden">
      ...
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 relative z-10 space-y-10">
        ...
        {/* GenuAI Works Featured Card - Active Path or New Selection */}
        {hasActivePath ? (
          <div className="bg-white border-2 border-indigo-500/30 p-6 sm:p-8 rounded-3xl shadow-lg hover:shadow-xl transition-all duration-300">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-200 px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                  ...
                  <span>ACTIVE ASSESSMENT PATH IN PROGRESS</span>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                <button
                  type="button"
                  onClick={() => onSelect('my-assessment')}
                  className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-black px-6 py-3.5 rounded-2xl shadow-md hover:shadow-lg transition-all text-xs sm:text-sm uppercase tracking-wider w-full sm:w-auto cursor-pointer"
                >
          ...
          >
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-3">
                ...
                <p className="text-sm sm:text-base text-slate-600 max-w-2xl leading-relaxed font-normal">
                  Select target companies and roles. GenuAI Works aggregates locked company requirements, performs Common/Majority analysis, checks verified assessment reuse, and generates your role-aware dynamic path.
                </p>
              </div>
        ...
        {/* 3 Spacious Path Cards (Practice, Search, Career Profile) */}
